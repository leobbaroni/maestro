...
| HyperFrames deep skills (`hyperframes-animation`, `-creative`, `-core`, `-cli`, `-keyframes`, `-registry`, `-media`) **plus `media-use` and `figma`** | `/plugin install hyperframes@claude-plugins-official` — the suite is in Anthropic's official marketplace, so all 21 skills arrive in one command. Outside the plugin system: `npx skills add heygen-com/hyperframes --full-depth` | Optional. Maestro works without them but prefers their frame presets, motion rules, and CLI when present — see `references/companions.md`. Costs ~4,850 always-on tokens for 21 descriptions, so install it when you render video and skip it when you don't |

...
| **A multi-shot piece** — "a short film", "a sequence", "a music-video scene", any brief with an arc or a recurring character | **phased production** (`generative-production.md`) | Seven phases: config, beat sheet, cast + prop extraction, look lock, **storyboard approved as one artifact**, keystone still first, then motion off each approved still. Nothing generates until the board is signed off |
| "Remove the X", "same shot but without the Y", "keep everything else identical" | edit prompt (`generative-production.md`) | Name one change, say *keep everything else exactly the same*, never re-describe the scene. Runs against an approved original, never against another edit |
| A brief that implies several assets but no arc | asset manifest (`generative-direction.md`) | Orders the whole set, locks the cinema mode once across all of it, and carries the engine per row — before the first prompt |

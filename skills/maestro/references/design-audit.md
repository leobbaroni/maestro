...
| Gate | Smell | Fix |
|---|---|---|
| Italic headers | `font-style: italic` on any h1–h6/headline, or an italicized emphasis word inside an upright headline | Headers are roman; emphasis via weight, accent color, or a drawn underline. Italic survives in body copy and in quote-voice display idioms (pull-quote heroes, letter salutations), plus preserved brand identity (`design-direction.md`) |
...
| Nav fingerprint | Wordmark-left + 4–5 inline links + button-right + hairline + white bg | Derive the nav from the committed form rather than reaching for the default shape (`references/design-direction.md`); that shape only with 2 genuine destinations |
...
---
*Distilled from: impeccable (critique, audit, harden, extract — the leading house), genjutsu design-audit, taste-skill (anti-slop core). Slop-test gates absorbed from hallmark before it was retired as a tracked source.*
